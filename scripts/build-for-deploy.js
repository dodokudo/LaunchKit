...
async function main() {
  ...
  // 3.6. seminar3/assetsをdist/seminar3/にコピー
  console.log('Copying seminar3 LP...');
  const seminar3Src = path.join(projectRoot, 'seminar3', 'assets');
  const seminar3Dist = path.join(distDir, 'seminar3');
  if (await fs.pathExists(seminar3Src)) {
    await fs.ensureDir(seminar3Dist);
    await fs.copy(seminar3Src, seminar3Dist);
  }

  // 3.7. seminar4/assetsをdist/seminar4/にコピー
  console.log('Copying seminar4 LP...');
  const seminar4Src = path.join(projectRoot, 'seminar4', 'assets');
  const seminar4Dist = path.join(distDir, 'seminar4');
  if (await fs.pathExists(seminar4Src)) {
    await fs.ensureDir(seminar4Dist);
  ...
  const course1Dist = path.join(distDir, 'course1');
  if (await fs.pathExists(course1Src)) {
    await fs.ensureDir(course1Dist);
    await fs.copy(course1Src, course1Dist);
  }

  // 3.8. threads-guide をdist/threads-guide/にコピー
  console.log('Copying threads-guide...');
  const threadsGuideSrc = path.join(projectRoot, 'threads-guide');
  const threadsGuideDist = path.join(distDir, 'threads-guide');
  if (await fs.pathExists(threadsGuideSrc)) {
    await fs.ensureDir(threadsGuideDist);
